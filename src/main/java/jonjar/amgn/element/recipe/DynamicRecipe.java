...
public class DynamicRecipe {
    ...
    public static JsonObject createPulverizeRecipeJson(Identifier input,Identifier output,String input_type,int time,float exp,int level){
        JsonObject  json = new JsonObject();


        json.addProperty("type", Amgn.MODID+":pulverize");

        JsonObject ingredient = new JsonObject();

        ingredient.addProperty(input_type,input.toString());

        json.add("ingredient",ingredient);

        json.addProperty("result", output.toString());
        json.addProperty("pulverizetime", time);
    ...
    public static JsonObject createPulverizeRecipeJson(Identifier input,Identifier output,String input_type,int time,int level){
        return createPulverizeRecipeJson( input, output, input_type, time, 0, level);
    }
